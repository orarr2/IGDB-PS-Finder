...
| `0007_hidden_gems_use_vector_index.sql` | Rewrites `get_hidden_gems` to take the 1000 nearest screenshots via the HNSW index first, then aggregate - fixes the statement-timeout (error 57014) the full-table scan caused. |
...
- `match_games_by_clip_oss`, `match_games_by_clip` - from **0006**
- `get_hidden_gems` - from **0007**
