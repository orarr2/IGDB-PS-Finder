...
(async () => {
  ...
  // Photo search runs fully on-device (transformers.js CLIP) and matches via the
  // match_games_by_clip_oss RPC. We can't run the model in jsdom, so we exercise
  // the results renderer directly and just assert the entry button is present.
  ...
  console.log("  ✓ search-by-photo button present; results screen renders (grid + hidden toggle)");
